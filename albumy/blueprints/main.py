...
def upload():
    print(f"Upload request: method={request.method}, files={request.files.keys()}")
    ...
    if request.method == 'POST' and 'file' in request.files:
        ...
        allowed_extensions = {'.jpg', '.jpeg', '.png', '.gif', '.bmp', '.avif', '.webp'}
        file_ext = os.path.splitext(f.filename)[1].lower()
        if file_ext not in allowed_extensions:
            print(f"Rejected file type: {file_ext}")
            flash(f'Unsupported file format. Please upload {", ".join(allowed_extensions)} files.', 'warning')
            return redirect(url_for('.upload'))
        
        # Generate new filename with .jpg extension
        base_filename = rename_image(f.filename)
        if file_ext in ['.avif', '.webp']:
            # Change extension to .jpg for unsupported formats
        ...
        try:
            ...
            # If conversion fails, try to rename the temp file
            if os.path.exists(temp_path):
                os.rename(temp_path, file_path)
        
        # Open saved file for resizing
        with open(file_path, 'rb') as image_file:
            filename_s = resize_image(image_file, filename, current_app.config['ALBUMY_PHOTO_SIZE']['small'])
            
        with open(file_path, 'rb') as image_file:
            filename_m = resize_image(image_file, filename, current_app.config['ALBUMY_PHOTO_SIZE']['medium'])
        photo = Photo(
            filename=filename,
        ...
        # Generate ML features (add this after the file is saved)
        # image_path is already defined as file_path above
        
        # Generate alt text
        try:
            photo.alt_text = ml_service.generate_alt_text(file_path)
        except Exception as e:
            print(f"ML alt text generation failed: {e}")
            photo.alt_text = "Uploaded image"
        
        # Detect objects for search
        try:
            detected_objects = ml_service.detect_objects(file_path)
            photo.detected_objects = json.dumps(detected_objects)
        except Exception as e:
            print(f"ML object detection failed: {e}")
            photo.detected_objects = json.dumps([])
        
        db.session.add(photo)
        db.session.commit()
        
        print(f"Photo saved successfully: ID={photo.id}, filename={photo.filename}")
        return '', 204  # Return empty response for Dropzone
